...
Для сравнения методов SLAM и оценки их применения необходимо выбрать критерий по которому будет оцениваться качество построенной карты. Сравннение облоков точек с моделью мира из симуялции является наиболее точным вариантом, но для этого необходим трудозатратный алгоритм, который требует критически большой вачислительной мощности. 

Метод косвенной оценки построенной карты является оценка траектории, построенной алгоритмом SLAM. Так как, траетория строится на основе одометрии и облака точек, а также корректируется при распазновании замкнутых петель и ключевых кадров, данный метод позволяет оценить в полном объеме алгоритм SLAM и косвено оценить качество построенной карты. 

Для качественной оценки траетории существуют ряд критериев (ссылка на критерии). Данные критерии можно деляться на две категории. Первая категория - для оценки результатов необходимо иметь данные истенной тректории, в контесте данной работе, истиннной траекторией является траектория которую считает симулятор Gazebo. Данные критерии явлеются наиболее точные, но их не всегда возможно использовать. Вторая категория - критерии, которым не требуется истиная трактория, чаще всего используюется вероятностная оценка, зависящая от облака точек лидара (тоже можно ссылку). 

В данной работе есть возможность использовать критерий, требующий истиный данных, поэтому был выбран критерий абсолютная ошибка траетории ATE (Absolute Trajectory Error) (ссылка на статью по АТЕ).  
